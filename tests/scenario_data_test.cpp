...
TEST_CASE("ship classes resolve the base-sprite owner and target pict",
          "[scenario][data]") {
  ScenarioData data;
  REQUIRE(data.LoadFromArchives());

  // base_sprite_clone_source_ship_class (+0xa08) is the first ship class whose
  // sh\x8an BaseImageID matches this class's (ShipClass_LoadShipClassVisualAnd
  // LaunchData 0x004b4ee0's clone branch; Ship_InitGameplayDataTables seeds it
  // to the class's own id, so a class with unique art points at itself).
  // Verified from the raw sh\x8an payloads (BaseImageID at +0x00):
  ...
  const auto owner_of = [&](std::int16_t raw_id) {
    const ShipClass *cls = data.Ship(raw_id);
    return cls ? cls->base_sprite_clone_source_ship_class : -2;
  };
  CHECK(owner_of(0x90) == 16);   // first Fed Viper owns its sprites
  CHECK(owner_of(0xe0) == 16);   // duplicate Fed Viper clones 16
  CHECK(owner_of(0xb4) == 52);   // first Rebel Dragon owns its sprites
  CHECK(owner_of(0xb7) == 52);   // second Rebel Dragon clones 52
  CHECK(owner_of(0xb8) == 53);   // second Rebel Destroyer clones 53
  CHECK(owner_of(0xb9) == 40);   // duplicate Wraith (Adult) clones 40
  CHECK(owner_of(0xbc) == 0);    // second Shuttle clones 0
  CHECK(owner_of(0x112) == 146); // Rebel Lightning: own art, own source

  // target_pict_ship_class (+0xa0c) is the class whose 3000+id target-info
  // PICT is drawn (NovaData_LoadAllShipClassVisualAndLaunchData 0x004aeda0):
  // the class itself when it owns that PICT, otherwise its base-sprite owner.
  // The shipped duplicates (e.g. 3096) have no own PICT, so they resolve to
  // the originals' existing PICTs (3016/3052/3053/3040/3000).
  const auto target_pict_of = [&](std::int16_t raw_id) {
    const ShipClass *cls = data.Ship(raw_id);
    return cls ? cls->target_pict_ship_class : -2;
  };
  CHECK(target_pict_of(0x90) == 16);
  CHECK(target_pict_of(0xe0) == 16);
  CHECK(target_pict_of(0xb4) == 52);
  CHECK(target_pict_of(0xb7) == 52);
  CHECK(target_pict_of(0xb8) == 53);
  CHECK(target_pict_of(0xb9) == 40);
  CHECK(target_pict_of(0xbc) == 0);
  CHECK(target_pict_of(0x112) == 146);
  CHECK(target_pict_of(0x90) + 3000 == 3016);
  CHECK(target_pict_of(0xe0) + 3000 == 3016);
}
