...
[[nodiscard]] ShipClass DecodeShip(std::span<const std::byte> bytes) {
  ...
  s.jump_duration_multiplier =
      JumpDurationMultiplierFromCapabilityFlags(s.capability_flags);
...
//   ItemClass, +0x3ee ScanMask, +0x3f0 BuyRandom, +0x3f2 RequireGovt.
[[nodiscard]] Outfit DecodeOutfit(std::span<const std::byte> bytes) {
  ...
  o.scan_mask = ReadBe16(bytes, 0x3ee);           // ScanMask
...
//   voice_type_code +0, flags_primary +2, flags_secondary +4, scan_fine +6,
...
//   scan_mask +0x32, comm_name +0x34,
//   target_code +0x44, require_lo +0x54, require_hi +0x58, inh_jam1-4
//   +0x5c..+0x62, medium_name +0x64, theme_color RGB24 +0xa4, ship_color RGB24
//   +0xa8, interface_id +0xac, news_pic_id +0xae. The record name
//   (resource.map, via ResourceData_ReadEntryMetadata + StripSubtitleSuffix) is
//   the display name.
...
[[nodiscard]] Government DecodeGovernment(std::span<const std::byte> bytes) {
  ...
  g.flags_secondary = ReadBe16(bytes, 0x04);
  ...
  g.scan_mask = ReadBeI16(bytes, 0x32);
  ...
  g.require_lo = ReadBe32(bytes, 0x54);
  g.require_hi = ReadBe32(bytes, 0x58);
