...
| Engine-glow integer rise/fall | **PARTIAL — NPC:** `Ship_HandleShip` banks normalized time and replays its ordered jump `+3`, banking `+2`, and thrust/settle/fade mutations at the 21 ms raw-call cadence. Player mutations still run once per port update. | Apply the same raw-call policy to the distinct player branches after reconstructing them; do not replace their integer operations with a shared target interpolation. |
