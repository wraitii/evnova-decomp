...
Corrected bugs go through `game::BugFixPolicy` (`src/game/compatibility.hpp`), which is loaded from the port-only `EV Nova Extra Prefs.ini` `[bugfixes]` section at startup and edited live by the Extra Prefs dialog (see docs/preferences_keybindings.md). Most fixes are bundled under the single `safe` flag; the behavior-changing ones have their own toggle (`outfit_slot_balance`, `outfit_prices`, `cron_events`, `particle_fog`).
...
* (fixed) **Small asteroids disappear at high resolutions** unless their sprites are padded to roughly 100×100. Fixed in `SpaceflightView::WrapAsteroids`: the port derives the keep-alive window from the largest loaded asteroid frame span instead of the record's own span and wraps records in world space, so the original's despawn/ring-refill miss cannot recur. Not routed through `BugFixPolicy` (renderer-structural divergence, no per-record `Sprite`); see the `BUGFIX(original)` note at the site. 
* (fixed) **`FragCount = 1` produces zero fragments**, despite the documented formula implying exactly one. Fixed via `BugFixPolicy::safe` in `Asteroid_SpawnDestructionPackage` (0x00462550): keep the original roll and raise the floor baseline to one (`BUGFIX(original)`). 
...
* (fixed) **Rank discounts do not apply to outfits**, despite the rank field being documented as affecting ships and outfits. Fixed in `NovaLanded_OutfitPrice`: charge the allied-rank price. The tech-level markdown is deliberately *not* restored on outfits (only the shipyard applies it), so the `<= 5` cap bug below still shapes ship prices only. Tech-level bug remains active.
...
* (fixed) **Max-guns/max-turrets outfit bonuses do not stack.** The executable added the first ModType-45/46 value once per owning outfit definition, so N copies of one modifier gave its bonus once. Fixed in `Outfit_ClampOwnedCountToCurrentLimits` (0x004656a0) under `BugFixPolicy::outfit_slot_balance`: scale by the owned count, matching the Bible's per-item "add to max" and ModType 27's explicit per-copy rule. The shipped "Sigma Mount Reinforcement" has `Max` 1, so the bug was latent in the base scenario; the faithful behavior is kept when the policy is off. 
* (fixed) **`Hxxx`/probably `Exxx` ship changes omit carried fighters**, leaving the new ship's bays empty; the initial player ship has the same problem. Mechanism: the class loaded-ammo count is written at `ammo_or_energy_cost_code` with no mode-99 special case (C/E/H arm 0x00449370 and new-game seeding 0x00489d70), but a mode-99 carried-ship bay's `AmmoType` is the carried ship class id and every read/spend path (`Weapon_GetWeaponBurstAttempts` 0x0046f2c0, `Weapon_CanFireWeaponBank`, `Weapon_FirePlayerWeaponBank`) and `Player_ReplaceShipWithCapturedHull` (0x00423fa0) use the bay's own counter, so the newly seeded bay reads empty. 74 shipped stock loadouts are affected. The port keeps the count in the bay counter in `NovaWeapon_SeedBanksFromShipStock` and `NovaWeapon_AddShipClassStockBanks` (`BUGFIX(original)`); the original class-slot write is not reimplemented and the fix is now gated under `BugFixPolicy::safe`.
* (fixed) **Burst-weapon state initializes from the class's default loadout, not the ship's live weapons.** `Weapon_InitShipWeaponBursts` (0x00413810) tests `g_ship_class_defs[class].default_weapon_ammo[bank] > 0`, so a burst bank armed only by a personality delta, capture or purchase never gets its burst counter cleared and reset cooldown preloaded. Fixed under `BugFixPolicy::safe` in `NovaWeapon_InitShipWeaponBursts` (`src/game/weapon_banks.cpp`) by testing the live mounted count; the class-default test is kept when the policy is off.
...
* **Only one point-defense weapon is selected per call, and the first ready bank gates the rest.** `Weapon_SelectTurretTargetWithinArc` (0x0043a310) picks the first ready mode-9/10 bank (lowest weapon id) and then searches for a target using only that weapon's reach and blind-spot; if it finds nothing it returns before any other PD bank is tried, and it fires at most one bank per call. A ship mounting several PD weapon types therefore sees the lowest-id one suppress the others -- e.g. a short-range PD laser can prevent a long-range PD turret from ever firing at a threat outside the laser's reach, giving the community-reported "multiple PD types interfere, sometimes none fire" symptom. Reproduced faithfully by `NovaWeapon_SelectTurretTargetWithinArc` (`src/game/weapon.cpp`); regression test `point defense only evaluates the first ready bank` (`tests/weapon_test.cpp`). 
...
* (fixed, gated) **Crön date ranges split day/month and year ranges incorrectly**, so a range such as 1/1/1178–1/1/1179 only fires on two calendar dates rather than throughout the interval. Corrected in `CronEventDateWindowAllows` under `BugFixPolicy::cron_events` (the month/day bound applies only at the boundary year); the original is reproduced when the policy is off. 
* (fixed, gated) **`Random` 0 still fires a crön.** The per-day roll is `NovaRandom_Range(0x65)` = 0..100 inclusive tested with `<=`, so `Random` 0 activates on roll 0 (~1/101 of eligible days) and the effective odds are `(Random+1)/101`. Corrected under `BugFixPolicy::cron_events` (rolls 1..100). 
* (fixed, gated) **A zero-duration crön runs its OnEnd script twice.** After the OnStart+OnEnd pair the slot stays active with a zero holdoff, so the next daily tick runs OnEnd again before deactivating (49 of the 125 shipped cröns use `Duration` 0). Corrected under `BugFixPolicy::cron_events`. 
* (fixed, gated) **A crön's post-end wait is loaded from `PreHoldoff`, not `PostHoldoff`.** On duration expiry the engine re-arms the holdoff counter from `PreHoldoff` (0x004395d9 reads block +0x22) whenever `PostHoldoff > 0`. With `PreHoldoff == 0` the slot never deactivates and re-runs OnEnd every day; with `PreHoldoff > 0` it re-runs the full OnStart+OnEnd pair after the wait. The duration-countdown arm also leaves `duration_counter` at 0, so the post-wait arm re-activates instead of retiring the slot. Shipped `V Rare Aur/Fed Syst Change` (0x00b8/0x00b9) and `Reb/Fed Syst Change 1` (0x00c0) therefore only ever fire once per game. Fixed in `Mission_TickDailyCronEvents` under `BugFixPolicy::cron_events` (uses the Bible's `PostHoldoff`, and latches `duration_counter = -1` after OnEnd); the original is reproduced when the policy is off. 
...
* (fixed) **A mission that aborts itself in `OnAccept` and starts another mission displays the second briefing twice.** Mechanism: the mission-script engine scans the shared `g_reaction_script_buffer` (DAT_007c8a10) and re-reads its length every byte (0x00449370), while the `S` opcode's activation runs the new mission's `OnAccept` payload through `Mission_RunMisnScriptPayload` (0x00448050) into that same buffer, so the outer scan continues over nested-payload bytes. Fixed in `Mission_ExecuteReactionScript`: the port passes an explicit `std::string_view`, so the outer script completes normally and the second briefing is shown once. Not gated by `BugFixPolicy` yet and there is no path that reproduces the broken behaviour; see the `BUGFIX(original)` note at the site and the row for 0x00448050 in `decomp-progress.tsv`. 
