...
- `POST /function/parent-view` — read-only overview of a parent with regions collapsed into calls to their synthetic views, plus each view decompiled: `{function|addr, regions:[{entry, stops, name?, ...}], names?, bodies?, plan?, timeout?}`; `plan:true` reports the recovered signatures without decompiling
