...
- Port-site marker: `// @port 0xADDR[,0xADDR…] NN% [tag[,tag…]]` at the primary port site (a header counts when the port is a header inline). Addresses are Ghidra function entry points. The tag CSV is defined at `PORT_TAGS` in `tools/ref_audit.py`: domain tags name unported work (`gameplay`, `rng`, `correctness`, `cadence`, `ui`, `rendering`, `audio`, `license`), status tags mark decisions (`verify`, `divergence`, `moddata`). Any `pct<100` row needs a domain tag; a `divergence`/`moddata` row with no domain tag is 100%. Keep the marker short; put narrative detail in the adjacent `// Ghidra …` comment. `tools/ref_audit.py` fails on unknown addresses, file mismatches, pct mismatches, unrecognized tags, and a permanent decision below 100%.
