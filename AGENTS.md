...
- Estimate remaining work conservatively: **0%** unported, regardless of Ghidra annotations; **10–30%** skeleton/cadence only; **40–90%** substantial but incomplete; **100%** scope reconstructed. `pct` counts only *unported work*; a known deviation or uncertainty is a tag, not a gap. A status tag (`divergence`, `moddata`) is a decision we expect to keep permanently, so a row whose only open item is one is **100%**. A temporary deviation is `gameplay` or `correctness` and keeps its pct.
...
- Port-site marker: `// @port 0xADDR[,0xADDR…] NN% [tag[,tag…]]` at the primary port site (a header counts when the port is a header inline). Addresses are Ghidra function entry points, except synthetic regions (interior labels of a collapsed parent), which carry the `synthetic` tag and list the interior address. The tag CSV is defined at `PORT_TAGS` in `tools/ref_audit.py`: domain tags name unported work (`gameplay`, `rng`, `correctness`, `cadence`, `ui`, `rendering`, `audio`, `license`), status tags mark decisions (`verify`, `divergence`, `moddata`), and `synthetic` is structural (the address is not a function entry, so Ghidra name/citation checks are skipped). Any `pct<100` row needs a domain tag; a `divergence`/`moddata` row with no domain tag is 100%. Keep the marker short; put narrative detail in the adjacent `// Ghidra …` comment. `tools/ref_audit.py` fails on unknown addresses, file mismatches, pct mismatches, unrecognized tags, and a permanent decision below 100%.
...
- Default iteration: `cmake --build build/release`; run relevant tests with `ctest --preset release -R '<pattern>'` (omit `-R` for the full suite). The preset sets `CTEST_PARALLEL_LEVEL` for auto-width parallel execution; a bare `ctest --test-dir build/release` runs serially, so add `-j` when invoking `ctest` directly.
